...
import { PiArchiveBoxLight, PiArchiveLight, PiDotsThreeOutlineThin, PiListChecksLight, PiTrashLight } from "react-icons/pi";
      ...
      renderCell: (params) => (
        <span
          ...
          ${params.row?.status == "completed" ? "border-green-500 text-green-500" :
            params.row?.status == "notStarted" ? "border-red-400 text-red-400" : 
            params.row?.status == "onHold" ? "border-indigo-400 text-indigo-500" : 
            params.row?.status == "inProgress" ? "border-yellow-400 text-yellow-400" : "border-rose-700 text-rose-700"}`}>
          <span>
            {params.row?.status == "completed" ? <div>Completed</div> : 
            params.row?.status == "notStarted" ? <div>Not Started</div> : 
            params.row?.status == "onHold" ? <div>On Hold</div> : 
      ...
      renderCell: (params) => (
        <div className="flex gap-[10px] items-center transition-all">
          ...
          <Tooltip arrow placement="top" title="Update Status">
            {" "}
            <CiCircleCheck onClick={() => handleOpenStatusModal(params.row)} className="cursor-pointer text-blue-500 text-[23px] hover:text-blue-600" />
          </Tooltip>
